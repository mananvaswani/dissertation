...
\paragraph{a) Iterating $\mathbf{\delta}$ over Gray code:} In order to iterate over values of $\mathbf{\delta} \in \{-1, 1\}^{m-1}$ with $\delta_1 = 1$ in Gray code order (Section \ref{sec:gray_code}), the code uses the help of two additional variables, \mintinline{c++}{int j} and \mintinline{c++}{arma::ivec g}. \mintinline{c++}{arma::ivec} is a vector of integers from Armadillo. \mintinline{c++}{j} is the active index i.e. to go from one value in the Gray code to the next, the $j^\text{th}$ bit is flipped. Additionally, the original code uses an auxiliary array \mintinline{c++}{g} to change from one value of  \mintinline{c++}{j} to the next. The condition of the \mintinline{c++}{while} loop ensures that all the gray codes of size $n-2$ are iterated over. $\mathbf{\delta}$ is represented by a boolean valued vector \mintinline{c++}{d}, and \mintinline{c++}{true} is considered equivalent to 1, and \mintinline{c++}{false} equivalent to -1.
